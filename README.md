...
| 平台 | 位址 | 連接埠 | 說明 |
| --- | --- | --- | --- |
| Java Edition | `100.74.51.85` | `25565` | 使用 Java 版加入。ViaVersion 支援較新版 Java 用戶端連線。 |
| Bedrock Edition | `100.74.51.85` | `19132`（UDP） | 使用基岩版的「伺服器」頁面手動新增。 |

> `100.74.51.85` 是 Tailscale 私有網路位址；從外網加入前，裝置必須先加入相同的 Tailscale 網路。
...
- 如基岩版無法加入，請先確認裝置已連線到 Tailscale，並確認使用 UDP `19132` 連接埠。
